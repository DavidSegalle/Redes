...
public:
    ...
    bool calculateSha(char* filename, char sha[SHA256_DIGEST_LENGTH]);

    bool getFile(char* dst, char* filename, int index);

    int getFileChunkCount(char* filename);

    int getLastChunkSize();
    
private:

    bool checkLoaded(char* filename);
